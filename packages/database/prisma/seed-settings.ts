import { PrismaClient, SettingValueType } from '@prisma/client';

const prisma = new PrismaClient();

/**
 * Seed default system settings for NextPik E-commerce Platform
 */

const defaultSettings = [
  // GENERAL / SYSTEM SETTINGS
  {
    key: 'site_name',
    category: 'general',
    value: 'NextPik E-commerce',
    valueType: SettingValueType.STRING,
    label: 'Site Name',
    description: 'The name of your e-commerce platform',
    isPublic: true,
    isEditable: true,
    requiresRestart: false,
    defaultValue: 'NextPik E-commerce',
  },
  {
    key: 'site_tagline',
    category: 'general',
    value: 'Where Elegance Meets Excellence',
    valueType: SettingValueType.STRING,
    label: 'Site Tagline',
    description: 'A short tagline for your brand',
    isPublic: true,
    isEditable: true,
    requiresRestart: false,
    defaultValue: 'Where Elegance Meets Excellence',
  },
  {
    key: 'contact_email',
    category: 'general',
    value: 'support@luxury.com',
    valueType: SettingValueType.STRING,
    label: 'Contact Email',
    description: 'Primary contact email address',
    isPublic: true,
    isEditable: true,
    requiresRestart: false,
    defaultValue: 'support@luxury.com',
  },
  {
    key: 'timezone',
    category: 'general',
    value: 'America/New_York',
    valueType: SettingValueType.STRING,
    label: 'Timezone',
    description: 'Default timezone for the platform',
    isPublic: false,
    isEditable: true,
    requiresRestart: true,
    defaultValue: 'America/New_York',
  },
  {
    key: 'maintenance_mode',
    category: 'general',
    value: false,
    valueType: SettingValueType.BOOLEAN,
    label: 'Maintenance Mode',
    description: 'Enable maintenance mode (disables all transactions)',
    isPublic: true,
    isEditable: true,
    requiresRestart: false,
    defaultValue: false,
  },
  {
    key: 'contact_phone',
    category: 'general',
    value: '+1-555-0123',
    valueType: SettingValueType.STRING,
    label: 'Contact Phone',
    description: 'Primary contact phone number',
    isPublic: true,
    isEditable: true,
    requiresRestart: false,
    defaultValue: '+1-555-0123',
  },
  {
    key: 'allowed_countries',
    category: 'general',
    value: ['US', 'CA', 'GB', 'FR', 'DE', 'AU', 'RW'],
    valueType: SettingValueType.ARRAY,
    label: 'Allowed Countries',
    description: 'Countries where shipping and orders are allowed',
    isPublic: true,
    isEditable: true,
    requiresRestart: false,
    defaultValue: ['US', 'CA', 'GB', 'FR', 'DE', 'AU', 'RW'],
  },

  // PAYMENT & ESCROW SETTINGS
  {
    key: 'escrow_enabled',
    category: 'payment',
    value: true,
    valueType: SettingValueType.BOOLEAN,
    label: 'Escrow Enabled',
    description: 'Enable escrow system (recommended for production)',
    isPublic: true,
    isEditable: true, // Editable but should be enabled in production
    requiresRestart: false,
    defaultValue: true,
  },
  {
    key: 'escrow_default_hold_days',
    category: 'payment',
    value: 7,
    valueType: SettingValueType.NUMBER,
    label: 'Default Escrow Hold Days',
    description: 'Number of days to hold funds after delivery confirmation',
    isPublic: false,
    isEditable: true,
    requiresRestart: false,
    defaultValue: 7,
  },
  {
    key: 'min_payout_amount',
    category: 'payment',
    value: 50,
    valueType: SettingValueType.NUMBER,
    label: 'Minimum Payout Amount',
    description: 'Minimum balance required for seller payout (USD)',
    isPublic: false,
    isEditable: true,
    requiresRestart: false,
    defaultValue: 50,
  },
  {
    key: 'payout_schedule',
    category: 'payment',
    value: 'weekly',
    valueType: SettingValueType.STRING,
    label: 'Payout Schedule',
    description: 'How often payouts are processed',
    isPublic: false,
    isEditable: true,
    requiresRestart: false,
    defaultValue: 'weekly',
  },
  {
    key: 'escrow_auto_release_enabled',
    category: 'payment',
    value: true,
    valueType: SettingValueType.BOOLEAN,
    label: 'Auto-Release Escrow',
    description: 'Automatically release funds after hold period expires',
    isPublic: false,
    isEditable: true,
    requiresRestart: false,
    defaultValue: true,
  },
  {
    key: 'payment_methods',
    category: 'payment',
    value: ['credit_card', 'stripe', 'paypal'],
    valueType: SettingValueType.ARRAY,
    label: 'Enabled Payment Methods',
    description: 'Payment methods available for checkout',
    isPublic: true,
    isEditable: true,
    requiresRestart: false,
    defaultValue: ['credit_card', 'stripe', 'paypal'],
  },

  // STRIPE PAYMENT INTEGRATION
  {
    key: 'stripe_enabled',
    category: 'payment',
    value: false,
    valueType: SettingValueType.BOOLEAN,
    label: 'Stripe Integration Enabled',
    description: 'Enable Stripe payment processing',
    isPublic: false,
    isEditable: true,
    requiresRestart: false,
    defaultValue: false,
  },
  {
    key: 'stripe_test_mode',
    category: 'payment',
    value: true,
    valueType: SettingValueType.BOOLEAN,
    label: 'Stripe Test Mode',
    description: 'Use Stripe test keys (sandbox environment)',
    isPublic: false,
    isEditable: true,
    requiresRestart: false,
    defaultValue: true,
  },
  {
    key: 'stripe_publishable_key',
    category: 'payment',
    value: '',
    valueType: SettingValueType.STRING,
    label: 'Stripe Publishable Key',
    description: 'Stripe public/publishable API key (safe to expose to frontend)',
    isPublic: true,
    isEditable: true,
    requiresRestart: false,
    defaultValue: '',
  },
  {
    key: 'stripe_secret_key',
    category: 'payment',
    value: '',
    valueType: SettingValueType.STRING,
    label: 'Stripe Secret Key',
    description: 'Stripe secret API key (NEVER expose to frontend, encrypted in database)',
    isPublic: false,
    isEditable: true,
    requiresRestart: false,
    defaultValue: '',
  },
  {
    key: 'stripe_webhook_secret',
    category: 'payment',
    value: '',
    valueType: SettingValueType.STRING,
    label: 'Stripe Webhook Secret',
    description: 'Stripe webhook signing secret for signature verification',
    isPublic: false,
    isEditable: true,
    requiresRestart: false,
    defaultValue: '',
  },
  {
    key: 'stripe_webhook_url',
    category: 'payment',
    value: '',
    valueType: SettingValueType.STRING,
    label: 'Stripe Webhook URL',
    description: 'The URL where Stripe sends webhook events (auto-configured)',
    isPublic: false,
    isEditable: false,
    requiresRestart: false,
    defaultValue: '',
  },
  {
    key: 'payment_capture_strategy',
    category: 'payment',
    value: 'ON_DELIVERY_WITH_FALLBACK',
    valueType: SettingValueType.STRING,
    label: 'Payment Capture Strategy',
    description:
      'When to capture authorized payments: IMMEDIATE, ON_DELIVERY, ON_DELIVERY_WITH_FALLBACK, or MANUAL',
    isPublic: false,
    isEditable: true,
    requiresRestart: false,
    defaultValue: 'ON_DELIVERY_WITH_FALLBACK',
  },
  {
    key: 'payment_auto_capture_day',
    category: 'payment',
    value: 6,
    valueType: SettingValueType.NUMBER,
    label: 'Auto-Capture Fallback Day',
    description:
      'Auto-capture on this day if not delivered (max 7 days before Stripe authorization expires)',
    isPublic: false,
    isEditable: true,
    requiresRestart: false,
    defaultValue: 6,
  },
  {
    key: 'stripe_currency',
    category: 'payment',
    value: 'USD',
    valueType: SettingValueType.STRING,
    label: 'Stripe Default Currency',
    description: 'Default currency for Stripe payment processing',
    isPublic: false,
    isEditable: true,
    requiresRestart: false,
    defaultValue: 'USD',
  },
  {
    key: 'stripe_capture_method',
    category: 'payment',
    value: 'manual',
    valueType: SettingValueType.STRING,
    label: 'Stripe Capture Method',
    description: 'Payment capture method (manual for escrow, automatic for instant capture)',
    isPublic: false,
    isEditable: true,
    requiresRestart: false,
    defaultValue: 'manual',
  },
  {
    key: 'stripe_statement_descriptor',
    category: 'payment',
    value: 'LUXURY ECOM',
    valueType: SettingValueType.STRING,
    label: 'Stripe Statement Descriptor',
    description: 'Text shown on customer credit card statements (max 22 chars)',
    isPublic: false,
    isEditable: true,
    requiresRestart: false,
    defaultValue: 'LUXURY ECOM',
  },
  {
    key: 'paypal_enabled',
    category: 'payment',
    value: false,
    valueType: SettingValueType.BOOLEAN,
    label: 'PayPal Enabled',
    description: 'Enable PayPal as a payment method',
    isPublic: false,
    isEditable: true,
    requiresRestart: false,
    defaultValue: false,
  },
  {
    key: 'paypal_brand_name',
    category: 'payment',
    value: 'NextPik',
    valueType: SettingValueType.STRING,
    label: 'PayPal Brand Name',
    description: 'Brand name shown in PayPal checkout',
    isPublic: false,
    isEditable: true,
    requiresRestart: false,
    defaultValue: 'NextPik',
  },
  {
    key: 'paypal_capture_method',
    category: 'payment',
    value: 'authorize',
    valueType: SettingValueType.STRING,
    label: 'PayPal Capture Method',
    description: 'How PayPal captures funds: authorize or capture',
    isPublic: false,
    isEditable: true,
    requiresRestart: false,
    defaultValue: 'authorize',
  },
  {
    key: 'stripe_auto_payout_enabled',
    category: 'payment',
    value: false,
    valueType: SettingValueType.BOOLEAN,
    label: 'Stripe Auto Payout',
    description: 'Automatically transfer funds to sellers via Stripe Connect',
    isPublic: false,
    isEditable: true,
    requiresRestart: false,
    defaultValue: false,
  },

  // PAYMENT PROCESSOR FEE SETTINGS
  {
    key: 'stripe_fee_percentage',
    category: 'payment',
    value: 2.9,
    valueType: SettingValueType.NUMBER,
    label: 'Stripe Fee Percentage',
    description: 'Stripe transaction fee percentage (e.g., 2.9 for 2.9%)',
    isPublic: false,
    isEditable: true,
    requiresRestart: false,
    defaultValue: 2.9,
  },
  {
    key: 'stripe_fee_fixed_eur',
    category: 'payment',
    value: 0.3,
    valueType: SettingValueType.NUMBER,
    label: 'Stripe Fixed Fee (EUR)',
    description: 'Stripe fixed fee for EUR transactions',
    isPublic: false,
    isEditable: true,
    requiresRestart: false,
    defaultValue: 0.3,
  },
  {
    key: 'stripe_fee_fixed_usd',
    category: 'payment',
    value: 0.3,
    valueType: SettingValueType.NUMBER,
    label: 'Stripe Fixed Fee (USD)',
    description: 'Stripe fixed fee for USD transactions',
    isPublic: false,
    isEditable: true,
    requiresRestart: false,
    defaultValue: 0.3,
  },
  {
    key: 'stripe_fee_fixed_gbp',
    category: 'payment',
    value: 0.2,
    valueType: SettingValueType.NUMBER,
    label: 'Stripe Fixed Fee (GBP)',
    description: 'Stripe fixed fee for GBP transactions',
    isPublic: false,
    isEditable: true,
    requiresRestart: false,
    defaultValue: 0.2,
  },
  {
    key: 'paypal_fee_percentage',
    category: 'payment',
    value: 3.49,
    valueType: SettingValueType.NUMBER,
    label: 'PayPal Fee Percentage',
    description: 'PayPal transaction fee percentage (e.g., 3.49 for 3.49%)',
    isPublic: false,
    isEditable: true,
    requiresRestart: false,
    defaultValue: 3.49,
  },
  {
    key: 'paypal_fee_fixed_eur',
    category: 'payment',
    value: 0.35,
    valueType: SettingValueType.NUMBER,
    label: 'PayPal Fixed Fee (EUR)',
    description: 'PayPal fixed fee for EUR transactions',
    isPublic: false,
    isEditable: true,
    requiresRestart: false,
    defaultValue: 0.35,
  },
  {
    key: 'paypal_fee_fixed_usd',
    category: 'payment',
    value: 0.3,
    valueType: SettingValueType.NUMBER,
    label: 'PayPal Fixed Fee (USD)',
    description: 'PayPal fixed fee for USD transactions',
    isPublic: false,
    isEditable: true,
    requiresRestart: false,
    defaultValue: 0.3,
  },
  {
    key: 'paypal_fee_fixed_gbp',
    category: 'payment',
    value: 0.3,
    valueType: SettingValueType.NUMBER,
    label: 'PayPal Fixed Fee (GBP)',
    description: 'PayPal fixed fee for GBP transactions',
    isPublic: false,
    isEditable: true,
    requiresRestart: false,
    defaultValue: 0.3,
  },
  {
    key: 'stripe_fee_fixed_aud',
    category: 'payment',
    value: 0.3,
    valueType: SettingValueType.NUMBER,
    label: 'Stripe Fixed Fee (AUD)',
    description: 'Stripe fixed fee for AUD (Australian Dollar) transactions',
    isPublic: false,
    isEditable: true,
    requiresRestart: false,
    defaultValue: 0.3,
  },
  {
    key: 'stripe_fee_fixed_cad',
    category: 'payment',
    value: 0.3,
    valueType: SettingValueType.NUMBER,
    label: 'Stripe Fixed Fee (CAD)',
    description: 'Stripe fixed fee for CAD (Canadian Dollar) transactions',
    isPublic: false,
    isEditable: true,
    requiresRestart: false,
    defaultValue: 0.3,
  },
  {
    key: 'stripe_fee_fixed_jpy',
    category: 'payment',
    value: 50,
    valueType: SettingValueType.NUMBER,
    label: 'Stripe Fixed Fee (JPY)',
    description: 'Stripe fixed fee for JPY (Japanese Yen) transactions',
    isPublic: false,
    isEditable: true,
    requiresRestart: false,
    defaultValue: 50,
  },
  {
    key: 'stripe_fee_fixed_chf',
    category: 'payment',
    value: 0.3,
    valueType: SettingValueType.NUMBER,
    label: 'Stripe Fixed Fee (CHF)',
    description: 'Stripe fixed fee for CHF (Swiss Franc) transactions',
    isPublic: false,
    isEditable: true,
    requiresRestart: false,
    defaultValue: 0.3,
  },
  {
    key: 'stripe_fee_fixed_rwf',
    category: 'payment',
    value: 300,
    valueType: SettingValueType.NUMBER,
    label: 'Stripe Fixed Fee (RWF)',
    description: 'Stripe fixed fee for RWF (Rwandan Franc) transactions',
    isPublic: false,
    isEditable: true,
    requiresRestart: false,
    defaultValue: 300,
  },
  {
    key: 'paypal_fee_fixed_aud',
    category: 'payment',
    value: 0.3,
    valueType: SettingValueType.NUMBER,
    label: 'PayPal Fixed Fee (AUD)',
    description: 'PayPal fixed fee for AUD (Australian Dollar) transactions',
    isPublic: false,
    isEditable: true,
    requiresRestart: false,
    defaultValue: 0.3,
  },
  {
    key: 'paypal_fee_fixed_cad',
    category: 'payment',
    value: 0.3,
    valueType: SettingValueType.NUMBER,
    label: 'PayPal Fixed Fee (CAD)',
    description: 'PayPal fixed fee for CAD (Canadian Dollar) transactions',
    isPublic: false,
    isEditable: true,
    requiresRestart: false,
    defaultValue: 0.3,
  },
  {
    key: 'paypal_fee_fixed_jpy',
    category: 'payment',
    value: 50,
    valueType: SettingValueType.NUMBER,
    label: 'PayPal Fixed Fee (JPY)',
    description: 'PayPal fixed fee for JPY (Japanese Yen) transactions',
    isPublic: false,
    isEditable: true,
    requiresRestart: false,
    defaultValue: 50,
  },
  {
    key: 'paypal_fee_fixed_chf',
    category: 'payment',
    value: 0.3,
    valueType: SettingValueType.NUMBER,
    label: 'PayPal Fixed Fee (CHF)',
    description: 'PayPal fixed fee for CHF (Swiss Franc) transactions',
    isPublic: false,
    isEditable: true,
    requiresRestart: false,
    defaultValue: 0.3,
  },
  {
    key: 'paypal_fee_fixed_rwf',
    category: 'payment',
    value: 300,
    valueType: SettingValueType.NUMBER,
    label: 'PayPal Fixed Fee (RWF)',
    description: 'PayPal fixed fee for RWF (Rwandan Franc) transactions',
    isPublic: false,
    isEditable: true,
    requiresRestart: false,
    defaultValue: 300,
  },
  {
    key: 'subscription_grace_days',
    category: 'payment',
    value: 3,
    valueType: SettingValueType.NUMBER,
    label: 'Subscription Grace Period (Days)',
    description: 'Days after payment failure before blocking subscription access (PAST_DUE status)',
    isPublic: false,
    isEditable: true,
    requiresRestart: false,
    defaultValue: 3,
  },

  // COMMISSION SETTINGS
  {
    key: 'global_commission_rate',
    category: 'commission',
    value: 15,
    valueType: SettingValueType.NUMBER,
    label: 'Global Commission Rate (%)',
    description: 'Default platform commission rate (percentage)',
    isPublic: false,
    isEditable: true,
    requiresRestart: false,
    defaultValue: 15,
  },
  {
    key: 'commission_type',
    category: 'commission',
    value: 'percentage',
    valueType: SettingValueType.STRING,
    label: 'Commission Type',
    description: 'How commission is calculated (percentage, fixed, or tiered)',
    isPublic: false,
    isEditable: true,
    requiresRestart: false,
    defaultValue: 'percentage',
  },
  {
    key: 'commission_applies_to_shipping',
    category: 'commission',
    value: false,
    valueType: SettingValueType.BOOLEAN,
    label: 'Apply Commission to Shipping',
    description: 'Include shipping costs in commission calculation',
    isPublic: false,
    isEditable: true,
    requiresRestart: false,
    defaultValue: false,
  },
  {
    key: 'commission_min_amount',
    category: 'commission',
    value: 0.5,
    valueType: SettingValueType.NUMBER,
    label: 'Minimum Commission Amount (USD)',
    description: 'Minimum commission charged per transaction regardless of rate',
    isPublic: false,
    isEditable: true,
    requiresRestart: false,
    defaultValue: 0.5,
  },
  {
    key: 'commission_max_amount',
    category: 'commission',
    value: 0,
    valueType: SettingValueType.NUMBER,
    label: 'Maximum Commission Amount (USD)',
    description: 'Maximum commission cap per transaction (0 = no maximum)',
    isPublic: false,
    isEditable: true,
    requiresRestart: false,
    defaultValue: 0,
  },
  {
    key: 'commission_fixed_fee',
    category: 'commission',
    value: 0.3,
    valueType: SettingValueType.NUMBER,
    label: 'Fixed Commission Fee (USD)',
    description: 'Fixed fee added to every transaction (similar to Stripe fee)',
    isPublic: false,
    isEditable: true,
    requiresRestart: false,
    defaultValue: 0.3,
  },
  {
    key: 'commission_default_rate',
    category: 'commission',
    value: 10,
    valueType: SettingValueType.NUMBER,
    label: 'Default Platform Commission (%)',
    description:
      'Default commission percentage charged on each transaction. Can be overridden per category or seller.',
    isPublic: true,
    isEditable: true,
    requiresRestart: false,
    defaultValue: 10,
  },

  // CURRENCY SETTINGS
  {
    key: 'default_currency',
    category: 'currency',
    value: 'USD',
    valueType: SettingValueType.STRING,
    label: 'Default Currency',
    description: 'Primary currency for the platform',
    isPublic: true,
    isEditable: true,
    requiresRestart: false,
    defaultValue: 'USD',
  },
  {
    key: 'supported_currencies',
    category: 'currency',
    value: ['USD', 'EUR', 'GBP', 'RWF'],
    valueType: SettingValueType.ARRAY,
    label: 'Supported Currencies',
    description: 'All currencies supported by the platform',
    isPublic: true,
    isEditable: true,
    requiresRestart: false,
    defaultValue: ['USD', 'EUR', 'GBP', 'RWF'],
  },
  {
    key: 'currency_auto_sync',
    category: 'currency',
    value: true,
    valueType: SettingValueType.BOOLEAN,
    label: 'Auto-Sync Exchange Rates',
    description: 'Automatically update exchange rates',
    isPublic: false,
    isEditable: true,
    requiresRestart: false,
    defaultValue: true,
  },
  {
    key: 'currency_sync_frequency',
    category: 'currency',
    value: 'daily',
    valueType: SettingValueType.STRING,
    label: 'Currency Sync Frequency',
    description: 'How often to sync exchange rates (hourly, daily, weekly)',
    isPublic: false,
    isEditable: true,
    requiresRestart: false,
    defaultValue: 'daily',
  },

  // TAX SETTINGS
  {
    key: 'tax_default_rate',
    category: 'tax',
    value: 0.1,
    valueType: SettingValueType.NUMBER,
    label: 'Default Tax Rate',
    description: 'Default tax rate as decimal (e.g., 0.10 = 10%, 0.15 = 15%)',
    isPublic: true,
    isEditable: true,
    requiresRestart: false,
    defaultValue: 0.1,
  },
  {
    key: 'tax_calculation_mode',
    category: 'tax',
    value: 'disabled',
    valueType: SettingValueType.STRING,
    label: 'Tax Calculation Mode',
    description:
      'How tax is calculated: disabled (no tax), simple (default rate), by_state (US state rates)',
    isPublic: true,
    isEditable: true,
    requiresRestart: false,
    defaultValue: 'disabled',
  },

  // SHIPPING SETTINGS
  {
    key: 'shipping_mode',
    category: 'shipping',
    value: 'hybrid',
    valueType: SettingValueType.STRING,
    label: 'Shipping Mode',
    description:
      'manual: Use configured rates | dhl_api: DHL integration | hybrid: DHL with manual fallback (Recommended)',
    isPublic: false,
    isEditable: true,
    requiresRestart: false,
    defaultValue: 'hybrid',
  },
  {
    key: 'shipping_standard_rate',
    category: 'shipping',
    value: 9.99,
    valueType: SettingValueType.NUMBER,
    label: 'Standard Shipping Rate',
    description: 'Base rate for standard shipping (USD)',
    isPublic: true,
    isEditable: true,
    requiresRestart: false,
    defaultValue: 9.99,
  },
  {
    key: 'shipping_express_rate',
    category: 'shipping',
    value: 19.99,
    valueType: SettingValueType.NUMBER,
    label: 'Express Shipping Rate',
    description: 'Base rate for express shipping (USD)',
    isPublic: true,
    isEditable: true,
    requiresRestart: false,
    defaultValue: 19.99,
  },
  {
    key: 'shipping_overnight_rate',
    category: 'shipping',
    value: 29.99,
    valueType: SettingValueType.NUMBER,
    label: 'Overnight Shipping Rate',
    description: 'Base rate for overnight delivery (USD)',
    isPublic: true,
    isEditable: true,
    requiresRestart: false,
    defaultValue: 29.99,
  },
  {
    key: 'shipping_international_surcharge',
    category: 'shipping',
    value: 15.0,
    valueType: SettingValueType.NUMBER,
    label: 'International Shipping Surcharge',
    description: 'Additional fee for international orders (USD)',
    isPublic: true,
    isEditable: true,
    requiresRestart: false,
    defaultValue: 15.0,
  },

  // DHL SHIPPING CONFIGURATION (Non-sensitive settings only)
  // SECURITY: API keys MUST be in .env, NEVER in database
  {
    key: 'dhl_api_environment',
    category: 'shipping',
    value: 'sandbox',
    valueType: SettingValueType.STRING,
    label: 'DHL API Environment',
    description:
      'DHL API environment: sandbox (for testing) or production (API keys stored in .env)',
    isPublic: false,
    isEditable: true,
    requiresRestart: true,
    defaultValue: 'sandbox',
  },
  {
    key: 'origin_country',
    category: 'shipping',
    value: 'US',
    valueType: SettingValueType.STRING,
    label: 'Origin Country Code',
    description: 'ISO 2-letter country code where shipments originate (e.g., US, GB, DE)',
    isPublic: false,
    isEditable: true,
    requiresRestart: false,
    defaultValue: 'US',
  },
  {
    key: 'origin_postal_code',
    category: 'shipping',
    value: '10001',
    valueType: SettingValueType.STRING,
    label: 'Origin Postal Code',
    description: 'Postal code of your warehouse/shipping location',
    isPublic: false,
    isEditable: true,
    requiresRestart: false,
    defaultValue: '10001',
  },
  {
    key: 'origin_street1',
    category: 'shipping',
    value: '123 Main Street',
    valueType: SettingValueType.STRING,
    label: 'Origin Street Address',
    description: 'Street address of your warehouse/shipping location',
    isPublic: false,
    isEditable: true,
    requiresRestart: false,
    defaultValue: '123 Main Street',
  },
  {
    key: 'origin_city',
    category: 'shipping',
    value: 'New York',
    valueType: SettingValueType.STRING,
    label: 'Origin City',
    description: 'City of your warehouse/shipping location',
    isPublic: false,
    isEditable: true,
    requiresRestart: false,
    defaultValue: 'New York',
  },
  {
    key: 'origin_state',
    category: 'shipping',
    value: 'NY',
    valueType: SettingValueType.STRING,
    label: 'Origin State/Province',
    description: 'State or province code of your warehouse/shipping location',
    isPublic: false,
    isEditable: true,
    requiresRestart: false,
    defaultValue: 'NY',
  },

  // DELIVERY SETTINGS
  {
    key: 'delivery_confirmation_required',
    category: 'delivery',
    value: true,
    valueType: SettingValueType.BOOLEAN,
    label: 'Delivery Confirmation Required',
    description: 'Require delivery confirmation to release escrow',
    isPublic: false,
    isEditable: false,
    requiresRestart: false,
    defaultValue: true,
  },
  {
    key: 'free_shipping_enabled',
    category: 'shipping',
    value: true,
    valueType: SettingValueType.BOOLEAN,
    label: 'Free Shipping Enabled',
    description: 'Enable free shipping for orders above threshold',
    isPublic: true,
    isEditable: true,
    requiresRestart: false,
    defaultValue: true,
  },
  {
    key: 'free_shipping_threshold',
    category: 'shipping',
    value: 100,
    valueType: SettingValueType.NUMBER,
    label: 'Free Shipping Threshold (USD)',
    description: 'Order total above which shipping is free',
    isPublic: true,
    isEditable: true,
    requiresRestart: false,
    defaultValue: 100,
  },
  {
    key: 'delivery_auto_assign',
    category: 'delivery',
    value: false,
    valueType: SettingValueType.BOOLEAN,
    label: 'Auto-Assign Deliveries',
    description: 'Automatically assign deliveries to available partners',
    isPublic: false,
    isEditable: true,
    requiresRestart: false,
    defaultValue: false,
  },
  {
    key: 'delivery_partner_commission',
    category: 'delivery',
    value: 10,
    valueType: SettingValueType.NUMBER,
    label: 'Delivery Partner Commission (%)',
    description: 'Commission rate for delivery partners',
    isPublic: false,
    isEditable: true,
    requiresRestart: false,
    defaultValue: 10,
  },

  // SHIPPING PROVIDER CONFIGURATION
  {
    key: 'shipping_primary_provider',
    category: 'delivery',
    value: 'EasyPost',
    valueType: SettingValueType.STRING,
    label: 'Primary Shipping Provider',
    description:
      'Primary shipping carrier for deliveries (EasyPost: multi-carrier, DHL: DHL Express only)',
    isPublic: true,
    isEditable: true,
    requiresRestart: false,
    defaultValue: 'EasyPost',
  },

  // NOTE: DHL API credentials (DHL_API_KEY, DHL_API_SECRET, DHL_TRACKING_ENABLED, DHL_API_BASE_URL, DHL_TRACKING_CACHE_TTL)
  // are now configured via environment variables (.env) for security reasons.
  // See apps/api/.env for DHL configuration.

  // EASYPOST SHIPPING INTEGRATION SETTINGS
  {
    key: 'easypost_enabled',
    category: 'delivery',
    value: true,
    valueType: SettingValueType.BOOLEAN,
    label: 'Enable EasyPost Shipping',
    description:
      'Enable EasyPost multi-carrier shipping integration (DEFAULT: Primary shipping provider)',
    isPublic: false,
    isEditable: true,
    requiresRestart: false,
    defaultValue: true,
  },
  {
    key: 'easypost_api_key',
    category: 'delivery',
    value: '',
    valueType: SettingValueType.STRING,
    label: 'EasyPost API Key',
    description: 'EasyPost API Key (use test key for development)',
    isPublic: false,
    isEditable: true,
    requiresRestart: true,
    defaultValue: '',
  },
  {
    key: 'easypost_test_mode',
    category: 'delivery',
    value: true,
    valueType: SettingValueType.BOOLEAN,
    label: 'EasyPost Test Mode',
    description: 'Use EasyPost test environment',
    isPublic: false,
    isEditable: true,
    requiresRestart: false,
    defaultValue: true,
  },
  {
    key: 'easypost_webhook_secret',
    category: 'delivery',
    value: '',
    valueType: SettingValueType.STRING,
    label: 'EasyPost Webhook Secret',
    description: 'EasyPost webhook HMAC secret for signature verification',
    isPublic: false,
    isEditable: true,
    requiresRestart: false,
    defaultValue: '',
  },
  {
    key: 'easypost_default_label_format',
    category: 'delivery',
    value: 'PNG',
    valueType: SettingValueType.STRING,
    label: 'Default Label Format',
    description: 'Default label format (PNG, PDF, ZPL, EPL2)',
    isPublic: false,
    isEditable: true,
    requiresRestart: false,
    defaultValue: 'PNG',
    validationRule: JSON.stringify({
      enum: ['PNG', 'PDF', 'ZPL', 'EPL2'],
    }),
  },
  {
    key: 'easypost_address_verification',
    category: 'delivery',
    value: true,
    valueType: SettingValueType.BOOLEAN,
    label: 'Address Verification',
    description: 'Enable address verification for shipping',
    isPublic: false,
    isEditable: true,
    requiresRestart: false,
    defaultValue: true,
  },
  {
    key: 'easypost_default_carriers',
    category: 'delivery',
    value: ['USPS', 'UPS', 'FedEx'],
    valueType: SettingValueType.ARRAY,
    label: 'Default Carriers',
    description: 'Default carriers to show rates for',
    isPublic: false,
    isEditable: true,
    requiresRestart: false,
    defaultValue: ['USPS', 'UPS', 'FedEx'],
  },

  // NOTE: EasyPost API credentials can also be configured via environment variables (.env)
  // for security reasons. See apps/api/.env for EasyPost configuration.

  // SELF-PICKUP SETTINGS (v2.10.0)
  {
    key: 'pickup_enabled',
    category: 'delivery',
    value: true,
    valueType: SettingValueType.BOOLEAN,
    label: 'Enable Self-Pickup',
    description: 'Allow customers to pick up orders from seller locations',
    isPublic: true,
    isEditable: true,
    requiresRestart: false,
    defaultValue: true,
  },
  {
    key: 'pickup_default_radius_km',
    category: 'delivery',
    value: 50,
    valueType: SettingValueType.INTEGER,
    label: 'Default Pickup Radius (km)',
    description: 'Default radius in kilometers for pickup availability (sellers can override)',
    isPublic: false,
    isEditable: true,
    requiresRestart: false,
    defaultValue: 50,
    validationRule: JSON.stringify({
      min: 1,
      max: 200,
    }),
  },
  {
    key: 'pickup_require_code_verification',
    category: 'delivery',
    value: true,
    valueType: SettingValueType.BOOLEAN,
    label: 'Require Pickup Code Verification',
    description: 'Require sellers to verify 6-digit pickup code when customer collects order',
    isPublic: false,
    isEditable: true,
    requiresRestart: false,
    defaultValue: true,
  },
  {
    key: 'pickup_expiration_days',
    category: 'delivery',
    value: 7,
    valueType: SettingValueType.INTEGER,
    label: 'Pickup Expiration (Days)',
    description: 'Number of days before uncollected pickup orders expire',
    isPublic: false,
    isEditable: true,
    requiresRestart: false,
    defaultValue: 7,
    validationRule: JSON.stringify({
      min: 1,
      max: 30,
    }),
  },
  {
    key: 'pickup_allow_scheduling',
    category: 'delivery',
    value: false,
    valueType: SettingValueType.BOOLEAN,
    label: 'Allow Pickup Scheduling',
    description: 'Allow customers to schedule a specific pickup time (future feature)',
    isPublic: false,
    isEditable: true,
    requiresRestart: false,
    defaultValue: false,
  },
  {
    key: 'pickup_default_fee',
    category: 'delivery',
    value: 0,
    valueType: SettingValueType.DECIMAL,
    label: 'Default Pickup Fee',
    description: 'Default fee for pickup orders (sellers can override)',
    isPublic: false,
    isEditable: true,
    requiresRestart: false,
    defaultValue: 0,
    validationRule: JSON.stringify({
      min: 0,
      max: 50,
    }),
  },

  // SECURITY SETTINGS
  {
    key: '2fa_required_for_admin',
    category: 'security',
    value: true,
    valueType: SettingValueType.BOOLEAN,
    label: 'Require 2FA for Admin',
    description: 'Enforce two-factor authentication for admin',
    isPublic: false,
    isEditable: true,
    requiresRestart: false,
    defaultValue: true,
  },
  {
    key: 'password_min_length',
    category: 'security',
    value: 8,
    valueType: SettingValueType.NUMBER,
    label: 'Minimum Password Length',
    description: 'Minimum characters required for passwords',
    isPublic: true,
    isEditable: true,
    requiresRestart: false,
    defaultValue: 8,
  },
  {
    key: 'session_timeout_minutes',
    category: 'security',
    value: 30,
    valueType: SettingValueType.NUMBER,
    label: 'Session Timeout (minutes)',
    description: 'Inactivity timeout for user sessions',
    isPublic: false,
    isEditable: true,
    requiresRestart: false,
    defaultValue: 30,
  },
  {
    key: 'max_login_attempts',
    category: 'security',
    value: 5,
    valueType: SettingValueType.NUMBER,
    label: 'Maximum Login Attempts',
    description: 'Failed login attempts before account lockout',
    isPublic: false,
    isEditable: true,
    requiresRestart: false,
    defaultValue: 5,
  },
  {
    key: 'password_require_special_chars',
    category: 'security',
    value: true,
    valueType: SettingValueType.BOOLEAN,
    label: 'Require Special Characters',
    description: 'Require special characters in passwords',
    isPublic: true,
    isEditable: true,
    requiresRestart: false,
    defaultValue: true,
  },
  {
    key: 'allowed_file_types',
    category: 'security',
    value: ['jpg', 'jpeg', 'png', 'gif', 'pdf', 'doc', 'docx'],
    valueType: SettingValueType.ARRAY,
    label: 'Allowed File Types',
    description: 'File extensions allowed for uploads',
    isPublic: true,
    isEditable: true,
    requiresRestart: false,
    defaultValue: ['jpg', 'jpeg', 'png', 'gif', 'pdf', 'doc', 'docx'],
  },
  {
    key: 'max_file_size_mb',
    category: 'security',
    value: 5,
    valueType: SettingValueType.NUMBER,
    label: 'Max File Size (MB)',
    description: 'Maximum file upload size in megabytes',
    isPublic: true,
    isEditable: true,
    requiresRestart: false,
    defaultValue: 5,
  },
  {
    key: 'email_verification_required',
    category: 'security',
    value: true,
    valueType: SettingValueType.BOOLEAN,
    label: 'Require Email Verification',
    description: 'Block login for users with unverified emails (excludes OAuth users)',
    isPublic: true,
    isEditable: true,
    requiresRestart: false,
    defaultValue: true,
  },
  {
    key: 'email_verification_grace_period_days',
    category: 'security',
    value: 7,
    valueType: SettingValueType.NUMBER,
    label: 'Email Verification Grace Period (Days)',
    description: 'Days after registration before enforcing email verification (0 = immediate)',
    isPublic: false,
    isEditable: true,
    requiresRestart: false,
    defaultValue: 7,
  },

  // ── 2FA ENFORCEMENT SETTINGS (v2.12.0) ─────────────────────────────────────
  // DEPRECATED: '2fa_required_for_admin' — kept for backward compat.
  // Source of truth for admin 2FA is now '2fa_required_for_admin_v2'.
  // Do not remove the old key; consumers may still read it.
  {
    key: '2fa_required_for_seller',
    category: 'security',
    value: true,
    valueType: SettingValueType.BOOLEAN,
    label: 'Require 2FA for Sellers',
    description: 'Enforce TOTP two-factor authentication for all seller accounts',
    isPublic: false,
    isEditable: true,
    requiresRestart: false,
    defaultValue: true,
  },
  {
    key: '2fa_required_for_admin_v2',
    category: 'security',
    value: true,
    valueType: SettingValueType.BOOLEAN,
    label: 'Require 2FA for Admins (v2)',
    description:
      'Enforce TOTP two-factor authentication for ADMIN and SUPER_ADMIN accounts (replaces deprecated 2fa_required_for_admin)',
    isPublic: false,
    isEditable: true,
    requiresRestart: false,
    defaultValue: true,
  },
  {
    key: '2fa_required_for_delivery_partner',
    category: 'security',
    value: true,
    valueType: SettingValueType.BOOLEAN,
    label: 'Require 2FA for Delivery Partners',
    description: 'Enforce TOTP two-factor authentication for all delivery partner accounts',
    isPublic: false,
    isEditable: true,
    requiresRestart: false,
    defaultValue: true,
  },
  {
    key: '2fa_grace_period_days_existing',
    category: 'security',
    value: 14,
    valueType: SettingValueType.NUMBER,
    label: '2FA Grace Period — Existing Accounts (Days)',
    description:
      'Days from first post-deploy login before 2FA becomes mandatory for existing accounts',
    isPublic: false,
    isEditable: true,
    requiresRestart: false,
    defaultValue: 14,
  },
  {
    key: '2fa_grace_period_days_new',
    category: 'security',
    value: 7,
    valueType: SettingValueType.NUMBER,
    label: '2FA Grace Period — New Accounts (Days)',
    description: 'Days from account creation before 2FA becomes mandatory for new accounts',
    isPublic: false,
    isEditable: true,
    requiresRestart: false,
    defaultValue: 7,
  },
  {
    key: '2fa_device_trust_duration_days',
    category: 'security',
    value: 30,
    valueType: SettingValueType.NUMBER,
    label: 'Trusted Device Duration (Days)',
    description: 'How long a trusted device token remains valid before requiring 2FA again',
    isPublic: false,
    isEditable: true,
    requiresRestart: false,
    defaultValue: 30,
  },
  {
    key: '2fa_device_trust_enabled',
    category: 'security',
    value: true,
    valueType: SettingValueType.BOOLEAN,
    label: 'Enable Trusted Devices',
    description: 'Allow users to skip 2FA on trusted devices for a configurable number of days',
    isPublic: false,
    isEditable: true,
    requiresRestart: false,
    defaultValue: true,
  },

  // NOTIFICATION SETTINGS
  {
    key: 'email_notifications_enabled',
    category: 'notifications',
    value: true,
    valueType: SettingValueType.BOOLEAN,
    label: 'Email Notifications',
    description: 'Enable email notifications globally',
    isPublic: false,
    isEditable: true,
    requiresRestart: false,
    defaultValue: true,
  },
  {
    key: 'sms_notifications_enabled',
    category: 'notifications',
    value: false,
    valueType: SettingValueType.BOOLEAN,
    label: 'SMS Notifications',
    description: 'Enable SMS notifications globally',
    isPublic: false,
    isEditable: true,
    requiresRestart: false,
    defaultValue: false,
  },
  {
    key: 'notification_events',
    category: 'notifications',
    value: [
      'order_placed',
      'order_shipped',
      'order_delivered',
      'payment_received',
      'payout_processed',
      'product_low_stock',
      'new_review',
      'account_login',
    ],
    valueType: SettingValueType.ARRAY,
    label: 'Notification Events',
    description: 'Events that trigger notifications',
    isPublic: false,
    isEditable: true,
    requiresRestart: false,
    defaultValue: [
      'order_placed',
      'order_shipped',
      'order_delivered',
      'payment_received',
      'payout_processed',
      'product_low_stock',
      'new_review',
      'account_login',
    ],
  },

  // INVENTORY SETTINGS
  {
    key: 'inventory.low_stock_threshold',
    category: 'inventory',
    value: 10,
    valueType: SettingValueType.NUMBER,
    label: 'Low Stock Threshold',
    description: 'Stock level that triggers low stock warning',
    isPublic: false,
    isEditable: true,
    requiresRestart: false,
    defaultValue: 10,
  },
  {
    key: 'inventory.auto_sku_generation',
    category: 'inventory',
    value: true,
    valueType: SettingValueType.BOOLEAN,
    label: 'Auto-Generate SKU',
    description: 'Automatically generate SKU for new products',
    isPublic: false,
    isEditable: true,
    requiresRestart: false,
    defaultValue: true,
  },
  {
    key: 'inventory.sku_prefix',
    category: 'inventory',
    value: 'PROD',
    valueType: SettingValueType.STRING,
    label: 'SKU Prefix',
    description: 'Prefix for auto-generated SKUs',
    isPublic: false,
    isEditable: true,
    requiresRestart: false,
    defaultValue: 'PROD',
  },
  {
    key: 'inventory.enable_stock_notifications',
    category: 'inventory',
    value: true,
    valueType: SettingValueType.BOOLEAN,
    label: 'Enable Stock Notifications',
    description: 'Send notifications when stock is low',
    isPublic: false,
    isEditable: true,
    requiresRestart: false,
    defaultValue: true,
  },
  {
    key: 'inventory.allow_negative_stock',
    category: 'inventory',
    value: false,
    valueType: SettingValueType.BOOLEAN,
    label: 'Allow Negative Stock',
    description: 'Allow orders when inventory is zero or negative',
    isPublic: false,
    isEditable: true,
    requiresRestart: false,
    defaultValue: false,
  },
  {
    key: 'inventory.transaction_history_page_size',
    category: 'inventory',
    value: 20,
    valueType: SettingValueType.NUMBER,
    label: 'Transaction History Page Size',
    description: 'Number of inventory transactions to display per page',
    isPublic: false,
    isEditable: true,
    requiresRestart: false,
    defaultValue: 20,
  },

  // SEO SETTINGS
  {
    key: 'seo_meta_title',
    category: 'seo',
    value: 'NextPik E-commerce - Premium Products Online',
    valueType: SettingValueType.STRING,
    label: 'SEO Meta Title',
    description: 'Default meta title for pages',
    isPublic: true,
    isEditable: true,
    requiresRestart: false,
    defaultValue: 'NextPik E-commerce - Premium Products Online',
  },
  {
    key: 'seo_meta_description',
    category: 'seo',
    value:
      'Discover our curated collection of luxury products. Premium quality, exceptional service, and secure shopping experience.',
    valueType: SettingValueType.STRING,
    label: 'SEO Meta Description',
    description: 'Default meta description for pages',
    isPublic: true,
    isEditable: true,
    requiresRestart: false,
    defaultValue:
      'Discover our curated collection of luxury products. Premium quality, exceptional service, and secure shopping experience.',
  },
  {
    key: 'seo_keywords',
    category: 'seo',
    value: 'luxury, premium, e-commerce, online shopping, high-end products',
    valueType: SettingValueType.STRING,
    label: 'SEO Keywords',
    description: 'Default SEO keywords (comma-separated)',
    isPublic: true,
    isEditable: true,
    requiresRestart: false,
    defaultValue: 'luxury, premium, e-commerce, online shopping, high-end products',
  },
  {
    key: 'analytics_enabled',
    category: 'seo',
    value: true,
    valueType: SettingValueType.BOOLEAN,
    label: 'Analytics Enabled',
    description: 'Enable Google Analytics and tracking',
    isPublic: false,
    isEditable: true,
    requiresRestart: false,
    defaultValue: true,
  },

  // SELLER MANAGEMENT & CREDITS SETTINGS
  {
    key: 'seller_monthly_credit_price',
    category: 'seller',
    value: 29.99,
    valueType: SettingValueType.NUMBER,
    label: 'Monthly Credit Price (USD)',
    description: 'Price charged per month of selling credits',
    isPublic: true,
    isEditable: true,
    requiresRestart: false,
    defaultValue: 29.99,
  },
  {
    key: 'seller_credit_grace_period_days',
    category: 'seller',
    value: 3,
    valueType: SettingValueType.NUMBER,
    label: 'Credit Depletion Grace Period (Days)',
    description: 'Days before suspending products after credits reach zero',
    isPublic: false,
    isEditable: true,
    requiresRestart: false,
    defaultValue: 3,
  },
  {
    key: 'seller_low_credit_warning_threshold',
    category: 'seller',
    value: 2,
    valueType: SettingValueType.NUMBER,
    label: 'Low Credit Warning Threshold (Months)',
    description: 'Send warning when credits fall below this number',
    isPublic: false,
    isEditable: true,
    requiresRestart: false,
    defaultValue: 2,
  },
  {
    key: 'seller_approval_email_enabled',
    category: 'seller',
    value: true,
    valueType: SettingValueType.BOOLEAN,
    label: 'Send Seller Approval Emails',
    description: 'Email sellers when approved/rejected',
    isPublic: false,
    isEditable: true,
    requiresRestart: false,
    defaultValue: true,
  },
  {
    key: 'seller_min_credit_purchase',
    category: 'seller',
    value: 1,
    valueType: SettingValueType.NUMBER,
    label: 'Minimum Credit Purchase (Months)',
    description: 'Minimum number of months sellers can purchase at once',
    isPublic: true,
    isEditable: true,
    requiresRestart: false,
    defaultValue: 1,
  },
  {
    key: 'seller_max_credit_purchase',
    category: 'seller',
    value: 12,
    valueType: SettingValueType.NUMBER,
    label: 'Maximum Credit Purchase (Months)',
    description: 'Maximum number of months sellers can purchase at once',
    isPublic: true,
    isEditable: true,
    requiresRestart: false,
    defaultValue: 12,
  },
  {
    key: 'seller_auto_approve',
    category: 'seller',
    value: false,
    valueType: SettingValueType.BOOLEAN,
    label: 'Auto-Approve Sellers',
    description: 'Automatically approve seller applications without manual review',
    isPublic: false,
    isEditable: true,
    requiresRestart: false,
    defaultValue: false,
  },
  {
    key: 'seller_application_review_required',
    category: 'seller',
    value: true,
    valueType: SettingValueType.BOOLEAN,
    label: 'Require Manual Review',
    description: 'Admin must manually review and approve each seller application',
    isPublic: false,
    isEditable: true,
    requiresRestart: false,
    defaultValue: true,
  },
  {
    key: 'seller_minimum_payout',
    category: 'seller',
    value: 50.0,
    valueType: SettingValueType.NUMBER,
    label: 'Minimum Payout Threshold (USD)',
    description: 'Minimum balance required before sellers can request a payout',
    isPublic: false,
    isEditable: true,
    requiresRestart: false,
    defaultValue: 50.0,
  },

  // GELATO PRINT-ON-DEMAND SETTINGS
  {
    key: 'gelato_enabled',
    category: 'fulfillment',
    value: false,
    valueType: SettingValueType.BOOLEAN,
    label: 'Gelato POD Enabled',
    description: 'Enable Gelato print-on-demand integration',
    isPublic: false,
    isEditable: true,
    requiresRestart: false,
    defaultValue: false,
  },
  {
    key: 'gelato_auto_submit_orders',
    category: 'fulfillment',
    value: true,
    valueType: SettingValueType.BOOLEAN,
    label: 'Auto-Submit POD Orders',
    description: 'Automatically submit POD orders to Gelato when payment is confirmed',
    isPublic: false,
    isEditable: true,
    requiresRestart: false,
    defaultValue: true,
  },
  {
    key: 'gelato_default_shipping_method',
    category: 'fulfillment',
    value: 'standard',
    valueType: SettingValueType.STRING,
    label: 'Default Gelato Shipping Method',
    description: 'Default shipping method for POD orders (standard, express, overnight)',
    isPublic: false,
    isEditable: true,
    requiresRestart: false,
    defaultValue: 'standard',
  },
  {
    key: 'gelato_markup_percentage',
    category: 'fulfillment',
    value: 30,
    valueType: SettingValueType.NUMBER,
    label: 'Gelato Markup Percentage',
    description: 'Default markup % on Gelato base cost for seller pricing suggestions',
    isPublic: false,
    isEditable: true,
    requiresRestart: false,
    defaultValue: 30,
  },
  {
    key: 'gelato_webhook_url',
    category: 'fulfillment',
    value: '',
    valueType: SettingValueType.STRING,
    label: 'Gelato Webhook URL',
    description: 'Webhook URL registered with Gelato (auto-populated)',
    isPublic: false,
    isEditable: true,
    requiresRestart: false,
    defaultValue: '',
  },
];

async function seedSettings() {
  console.log('🌱 Seeding system settings...');

  for (const setting of defaultSettings) {
    await prisma.systemSetting.upsert({
      where: { key: setting.key },
      update: {
        label: setting.label,
        description: setting.description,
        isPublic: setting.isPublic,
      },
      create: setting,
    });
    console.log(`  ✓ ${setting.key}`);
  }

  console.log(`✅ Seeded ${defaultSettings.length} system settings`);
}

seedSettings()
  .catch((e) => {
    console.error('❌ Error seeding settings:', e);
    process.exit(1);
  })
  .finally(async () => {
    await prisma.$disconnect();
  });
