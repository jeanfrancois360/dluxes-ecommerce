...
function SellerDetailContent() {
  ...
  const handleAdjustCredits = async () => {
    ...
    try {
      ...
      const creditRes = await fetch(`${API_URL}/admin/sellers/${params.id}/credits/history?limit=10`, {
        headers: {
          Authorization: `Bearer ${localStorage.getItem('auth_token')}`,
        },
      });
        ...
        <div className="flex items-center gap-4">
          ...
          <div>
            <div className="flex items-center gap-3 mb-1">
              <h1 className="text-2xl font-bold text-black">{seller.name}</h1>
              <StatusBadge status={seller.status} />
            </div>
            <p className="text-neutral-600">{seller.email}</p>
          </div>
        </div>
                ...
                <div className="flex gap-3">
                  ...
                  <div className="flex-1 pb-4">
                    ...
                    <p className="text-xs text-neutral-600 mt-1">
                      {format(new Date(seller.appliedAt || seller.createdAt), 'MMM d, yyyy h:mm a')}
                    </p>
                  </div>
                </div>
              )}
                  ...
                  >
                    <div className="flex items-center justify-between mb-1">
                      ...
                      >
                        ...
                        {transaction.amount} {Math.abs(transaction.amount) === 1 ? 'month' : 'months'}
                      </span>
