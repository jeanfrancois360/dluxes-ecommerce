...
function SubscriptionsDashboardContent() {
  ...
  } = useSWR('subscription-stats', () => subscriptionApi.adminGetStatistics());
  ...
  const {
    ...
    error: adError,
    isLoading: adLoading,
  } = useSWR('ad-stats', () => advertisementPlansApi.adminGetStatistics());
  ...
  React.useEffect(() => {
    if (error) {
      toast.error(t('errors.loadStatistics'));
    }
  }, [error, t]);
  ...
  const stats = React.useMemo(() => {
    ...
    const churnRate =
      sellerStats.canceledSubscriptions > 0
        ? Number(
            ((sellerStats.canceledSubscriptions / sellerStats.totalSubscriptions) * 100).toFixed(1)
          )
        : 0;
      ...
      <div className="space-y-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
          {[1, 2, 3, 4].map((i) => (
            <div
              key={i}
              className="bg-white rounded-lg shadow p-6 animate-pulse border border-gray-200"
            >
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="rounded-lg bg-gray-900 p-8">
        ...
        <p className="text-gray-300 text-base ml-14">{t('heroDescription')}</p>
      </div>

      {/* Primary Stats Grid */}
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        <StatCard
          title={t('stats.activeFeaturePlans')}
        ...
        />
        <StatCard
          title={t('stats.totalFeaturePlans')}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="bg-white rounded-lg shadow p-6 border border-gray-200">
          <div className="flex items-center gap-3 mb-6">
            ...
            <h3 className="text-lg font-bold text-gray-900">{t('sellerPlans.title')}</h3>
          </div>
          <div className="space-y-4">
              ...
              <div className="flex items-center gap-4">
                ...
                <div>
                  <p className="text-gray-900 font-bold text-base">
                    {t('sellerPlans.activeSubscribers')}
                  </p>
                  <p className="text-gray-600 text-sm font-medium">
                    {t('sellerPlans.subscribersDescription')}
                  </p>
                </div>
              </div>
              <div className="text-right">
                <p className="text-3xl font-bold text-gray-900 mb-1">
                  {stats.totalSellerSubscriptions}
                </p>
                <Link
                  href="/admin/subscriptions/sellers"
                  className="inline-flex items-center gap-1.5 text-[#CBB57B] text-sm hover:text-[#a89158] font-semibold"
                >
                  {t('sellerPlans.viewAll')}
        ...
        <div className="bg-white rounded-lg shadow p-6 border border-gray-200">
          <div className="flex items-center gap-3 mb-6">
            ...
            <h3 className="text-lg font-bold text-gray-900">{t('advertisementPlans.title')}</h3>
          </div>
          <div className="space-y-4">
            <div className="flex items-center justify-between p-5 bg-gray-50 rounded-lg border border-gray-200">
              <div className="flex items-center gap-4">
                ...
                <div>
                  <p className="text-gray-900 font-bold text-base">
                    {t('advertisementPlans.activeCampaigns')}
                  </p>
                  <p className="text-gray-600 text-sm font-medium">
                    {t('advertisementPlans.campaignsDescription')}
                  </p>
                </div>
              </div>
              <div className="text-right">
                <p className="text-3xl font-bold text-gray-900 mb-1">
                  {stats.totalAdvertisementSubscriptions}
                </p>
                <Link
                  href="/admin/subscriptions/advertisement"
                  className="inline-flex items-center gap-1.5 text-[#CBB57B] text-sm hover:text-[#a89158] font-semibold"
                >
                  {t('advertisementPlans.viewAll')}
                  <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path
                      strokeLinecap="round"
                      strokeLinejoin="round"
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          ...
          <div className="flex items-center justify-between">
            <div>
              <h3 className="text-base font-bold text-gray-900 mb-1">
                {t('quickActions.manageFeaturePlans.title')}
              </h3>
              <p className="text-sm text-gray-600">
                {t('quickActions.manageFeaturePlans.description')}
              </p>
            </div>
        ...
        >
          <div className="flex items-center justify-between">
            <div>
              <h3 className="text-base font-bold text-gray-900 mb-1">
                {t('quickActions.adCampaigns.title')}
              </h3>
              <p className="text-sm text-gray-600">{t('quickActions.adCampaigns.description')}</p>
            </div>
            <div className="p-3 bg-[#CBB57B]/10 rounded-lg">
              <svg
                className="w-6 h-6 text-[#CBB57B]"
        ...
        >
          <div className="flex items-center justify-between">
            <div>
              <h3 className="text-base font-bold text-gray-900 mb-1">
                {t('quickActions.sellerFeaturePlans.title')}
              </h3>
              <p className="text-sm text-gray-600">
                {t('quickActions.sellerFeaturePlans.description')}
              </p>
            </div>
            <div className="p-3 bg-[#CBB57B]/10 rounded-lg">
              <svg
