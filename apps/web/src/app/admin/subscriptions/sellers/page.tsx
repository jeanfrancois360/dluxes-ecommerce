...
function StatusBadge({ status }: { status: string }) {
  ...
  return (
    <span
      className={`inline-flex items-center gap-1.5 px-3 py-1.5 text-xs font-semibold rounded-lg ${badge.bg} ${badge.text} border border-current/20`}
    >
  ...
  return (
    <div className="space-y-2">
      <div className="flex items-center justify-between text-xs">
        ...
        <span
          className={`font-bold ${isDanger ? 'text-red-600' : isWarning ? 'text-yellow-600' : 'text-black'}`}
        >
          {current} / {max === -1 ? '∞' : max}
        </span>
      </div>
      <div className="h-2 bg-neutral-200 rounded-full overflow-hidden">
        <div
          className={`h-full transition-all duration-500 rounded-full ${
            isDanger ? 'bg-red-600' : isWarning ? 'bg-yellow-500' : 'bg-[#CBB57B]'
          }`}
...
function SellerSubscriptionsContent() {
  ...
  const stats = React.useMemo(
    () => ({
      ...
      revenue: subscriptions.reduce((sum, s) => {
        if (s.status === 'ACTIVE') {
          return (
            sum + (s.billingCycle === 'MONTHLY' ? s.plan.monthlyPrice : s.plan.yearlyPrice / 12)
          );
        }
        return sum;
      }, 0),
  ...
  if (isLoading) {
    return (
      <>
        <PageHeader
          title="Seller Feature Plans"
          description="View and manage sellers subscribed to feature plans (unlock premium product types)"
        />
  ...
  return (
    <>
      ...
      <div className="px-4 sm:px-6 lg:px-8 py-8 space-y-8">
        ...
        <section>
          <h2 className="text-lg font-semibold text-neutral-900 mb-4">Seller Subscriptions</h2>
            ...
            <div className="flex-1">
              <div className="relative">
                ...
                <input
                  ...
                  className="w-full pl-12 pr-4 py-3.5 bg-white border-2 border-neutral-200 rounded-xl text-black placeholder-neutral-500 focus:outline-none focus:border-[#CBB57B] transition-colors"
                />
              </div>
            </div>
          ...
          {subscriptions.length === 0 ? (
            <div className="bg-white rounded-xl shadow-sm border border-neutral-200 p-16 text-center">
              ...
              <h3 className="text-xl font-bold text-neutral-900 mb-2">No sellers found</h3>
            ...
            <>
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                {subscriptions.map((sub) => {
                  ...
                  const price =
                    sub.billingCycle === 'MONTHLY' ? sub.plan.monthlyPrice : sub.plan.yearlyPrice;
                  ...
                  return (
                    <div
                      key={sub.id}
                      className="bg-white rounded-xl shadow-sm border border-neutral-200 hover:border-[#CBB57B] hover:shadow-lg transition-all"
                    >
                      {/* Header with Avatar */}
                      <div className="p-6 border-b border-neutral-200">
                        <div className="flex items-start gap-4 mb-4">
                          {/* Avatar */}
                          <div className="flex-shrink-0 w-12 h-12 bg-black rounded-lg flex items-center justify-center text-white font-bold text-lg">
                        ...
                        <div className="flex items-center justify-between">
                          <div>
                            ...
                            <p className="text-lg font-bold text-black">{sub.plan.name}</p>
                      ...
                      <div className="p-6 bg-neutral-50 space-y-4">
                        ...
                        <ProgressBar current={creditsUsed} max={creditsMax} label="Credits Used" />
                      </div>
                        ...
                        <div className="space-y-3">
                          ...
                          <div className="flex items-center gap-3">
                            <div className="flex flex-col items-center">
                              ...
                              <div
                                className={`w-2.5 h-2.5 rounded-full ${daysUntilExpiry > 0 ? 'bg-green-600' : 'bg-red-600'}`}
                              />
                            </div>
                          ...
                          <div className="pt-3 border-t border-neutral-200 space-y-2">
                            <div className="flex items-center justify-between text-sm">
                              ...
                              <span
                                className={`font-bold ${daysUntilExpiry > 7 ? 'text-green-600' : daysUntilExpiry > 0 ? 'text-yellow-600' : 'text-red-600'}`}
                              >
                      ...
                      <div className="p-6 pt-0 flex gap-2">
                        ...
                        <button className="px-4 py-2.5 border-2 border-neutral-200 text-neutral-700 rounded-lg hover:bg-neutral-50 hover:border-neutral-300 transition-colors">
                          <svg
                            className="w-5 h-5"
                            fill="none"
                            stroke="currentColor"
                  ...
                  <div className="flex flex-col sm:flex-row items-center justify-between gap-4">
                    <p className="text-sm text-neutral-600 font-medium">
                      ...
                      <span className="font-bold text-black">
                        {(page - 1) * (pagination?.limit || 10) + 1}
                      </span>{' '}
                      to{' '}
                      <span className="font-bold text-black">
                        {Math.min(page * (pagination?.limit || 10), pagination?.total || 0)}
                      </span>{' '}
                      of <span className="font-bold text-black">{pagination?.total || 0}</span>{' '}
                      results
                    </p>
                    ...
                    <div className="flex items-center gap-2">
                      <button
                        ...
                        disabled={page === 1}
                        className="px-4 py-2.5 bg-white border-2 border-neutral-200 text-neutral-700 rounded-lg hover:bg-neutral-50 hover:border-neutral-300 disabled:opacity-50 disabled:cursor-not-allowed transition-colors font-semibold flex items-center gap-2"
                      >
                        <svg
                          className="w-4 h-4"
                          fill="none"
                            ...
                            return (
                              <button
                                ...
                                className={`w-10 h-10 rounded-lg font-semibold transition-colors ${
                                  page === pageNum
                                    ? 'bg-black text-white shadow-sm'
                                    : 'bg-white border-2 border-neutral-200 text-neutral-700 hover:bg-neutral-50 hover:border-neutral-300'
                                }`}
                              >
                                {pageNum}
                              </button>
