...
function DeliveryPayoutsContent() {
  ...
  const handleProcessPayout = async () => {
    ...
    try {
      const token = localStorage.getItem('auth_token');
      await axios.post(
        `${API_URL}/delivery-payouts/${selectedPayout.id}/process`,
        processData,
        {
          headers: { Authorization: `Bearer ${token}` },
        }
  ...
  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-3xl font-bold tracking-tight">Delivery Provider Payouts</h1>
        <p className="text-muted-foreground">Manage and process delivery partner payouts</p>
      </div>
