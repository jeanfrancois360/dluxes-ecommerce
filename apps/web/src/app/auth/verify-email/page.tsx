...
export default function VerifyEmailPage() {
  ...
  useEffect(() => {
    if (token === null) return; // not yet read from URL
    if (token) {
      verifyEmail(token);
    } else {
      setState('resend');
      ...
      <AuthLayout title={t('verifyingTitle')} subtitle={t('verifyingSubtitle')}>
        ...
        >
          ...
          <div className="space-y-2">
            <p className="text-neutral-600 text-base">{t('verifyingMessage')}</p>
            <p className="text-sm text-neutral-500">{t('onlyTakesMoment')}</p>
          </div>
        </motion.div>
      </AuthLayout>
    );
  ...
  if (state === 'success') {
    return (
      <AuthLayout title={t('emailVerified')} subtitle={t('successfullyVerified')}>
        <motion.div
        ...
        >
          ...
          <div className="space-y-2">
            <p className="text-neutral-600 text-base">{t('canSignIn')}</p>
            <p className="text-sm text-neutral-500">{t('redirectingToLogin')}</p>
          </div>
  ...
  if (state === 'expired') {
    return (
      <AuthLayout title={t('linkExpired')} subtitle={t('linkHasExpired')}>
        <div className="space-y-6">
          ...
          <div className="text-center">
            ...
            <p className="text-sm text-neutral-600 mb-6">{t('linksExpire24h')}</p>
          </div>
      ...
      <div className="space-y-6">
        ...
        <div className="text-center pt-4 border-t border-neutral-200 space-y-2">
          ...
          <div>
            ...
            <Link href="/" className="text-sm text-neutral-600 hover:text-gold transition-colors">
              {t('returnHome')}
            </Link>
          </div>
        </div>
