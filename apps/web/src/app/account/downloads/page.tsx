...
export default function MyDownloadsPage() {
  ...
  const downloadsByOrder = downloads.reduce(
    (acc, download) => {
        ...
        <div className="max-w-6xl mx-auto">
          ...
          >
            <div className="bg-white rounded-xl p-4 border border-neutral-100">
              ...
              <p className="text-sm text-neutral-600">{t('stats.totalFiles')}</p>
            </div>
            <div className="bg-white rounded-xl p-4 border border-neutral-100">
              <p className="text-2xl font-bold text-gold">{Object.keys(downloadsByOrder).length}</p>
              <p className="text-sm text-neutral-600">{t('stats.orders')}</p>
            </div>
            <div className="bg-white rounded-xl p-4 border border-neutral-100">
              <p className="text-2xl font-bold text-green-500">
                {downloads.filter((d) => d.canDownload).length}
              </p>
              <p className="text-sm text-neutral-600">{t('stats.available')}</p>
            </div>
          </motion.div>
          {downloads.length === 0 ? (
            /* Empty State */
                        ...
                        <div className="flex items-start gap-4">
                          ...
                          <div className="flex-1 min-w-0">
                            ...
                            <div className="flex flex-wrap items-center gap-3 mt-2 text-sm text-neutral-500">
                              ...
                              {download.digitalVersion && <span>v{download.digitalVersion}</span>}
                              {download.digitalLicenseType && (
                                <span className="capitalize">
                                  {t('license', { type: download.digitalLicenseType })}
                                </span>
                              )}
                            </div>
                          ...
                          <div className="flex-shrink-0">
                            <button
                              ...
                              disabled={
                                !download.canDownload || downloadingId === download.productId
                              }
