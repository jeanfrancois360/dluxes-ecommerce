...
export function CountrySelect({
  ...
  const handleKeyDown = (e: React.KeyboardEvent) => {
    ...
    if (e.key === 'ArrowDown') {
      ...
      e.preventDefault();
      if (filtered[highlighted]) handleSelect(filtered[highlighted].code);
      ...
      >
        {selected && !isOpen ? (
          <span className="flex-1 text-sm text-gray-900">
            <span className="font-mono text-gray-500 mr-2">{selected.code}</span>
            {selected.name}
          </span>
        ) : (
          <input
        ...
        <ul className="absolute z-50 mt-1 w-full bg-white border border-gray-200 rounded-lg shadow-lg max-h-60 overflow-y-auto">
          {filtered.length === 0 ? (
            ...
            filtered.map((country, idx) => (
              <li
                key={country.code}
                onMouseDown={() => handleSelect(country.code)}
              ...
              >
                <span className="font-mono text-xs text-gray-400 w-6 shrink-0">{country.code}</span>
