...
const NOTIFICATION_EVENTS = [
  ...
  },
  {
    ...
    description: 'When seller payout is sent',
  },
  {
    value: 'product_low_stock',
  ...
  { value: 'new_review', label: 'New Review', description: 'When a product receives a review' },
  {
    value: 'account_login',
    label: 'Account Login',
    description: 'When user logs in from new device',
  },
];
      ...
      >
        <SettingsToggle
          ...
          onCheckedChange={(checked) =>
            form.setValue('email_notifications_enabled', checked, { shouldDirty: true })
          }
        ...
        <SettingsToggle
          ...
          onCheckedChange={(checked) =>
            form.setValue('sms_notifications_enabled', checked, { shouldDirty: true })
          }
      ...
      >
            ...
            return (
              ...
              >
                <div className="space-y-0.5 flex-1">
                  <Label htmlFor={`event_${event.value}`} className="cursor-pointer font-medium">
                    {event.label}
                  </Label>
        ...
        <div className="rounded-lg border border-gray-200 bg-gray-50 p-4 ">
          ...
          <div className="text-sm text-muted-foreground space-y-1">
            <p>Email: {form.watch('email_notifications_enabled') ? 'Enabled' : 'Disabled'}</p>
            <p>SMS: {form.watch('sms_notifications_enabled') ? 'Enabled' : 'Disabled'}</p>
            <p>Events: {form.watch('notification_events')?.length || 0} selected</p>
          </div>
        </div>
      </SettingsCard>
