...
export function payoutCompletedTemplate(data: PayoutCompletedData): string {
  ...
  const completedAt = `${new Date(data.completedDate).toLocaleDateString()} ${new Date(data.completedDate).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}`;
  ...
  const content = `
    ...
    <p style="color: #4B5563; font-size: 15px; line-height: 1.65; margin: 0 0 28px 0; font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, 'Helvetica Neue', Arial, sans-serif;">
      Hello ${data.sellerName}, your payout for <strong style="color: #0A0A0A;">${data.storeName}</strong> has been successfully processed.
    </p>
          ...
          <table cellpadding="0" cellspacing="0" role="presentation" width="100%">
            ...
            ${
              data.paymentReference
                ? `
            <tr>
              <td style="padding: 6px 0; color: #6B7280; font-size: 13px; font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, 'Helvetica Neue', Arial, sans-serif;">Transaction reference</td>
              <td style="padding: 6px 0; text-align: right; color: #0A0A0A; font-size: 13px; font-family: 'Courier New', 'Lucida Console', monospace;">${data.paymentReference}</td>
            </tr>
            `
                : ''
            }
            ...
            <tr>
              <td style="padding: 10px 0 6px 0; color: #0A0A0A; font-size: 13px; font-weight: 600; border-top: 1px solid #E5E7EB; font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, 'Helvetica Neue', Arial, sans-serif;">Completed</td>
              <td style="padding: 10px 0 6px 0; text-align: right; color: #0A0A0A; font-size: 13px; font-weight: 600; border-top: 1px solid #E5E7EB; font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, 'Helvetica Neue', Arial, sans-serif;">${completedAt}</td>
            </tr>
          </table>
        </td>
      </tr>
    ...
    <p style="color: #9CA3AF; font-size: 13px; line-height: 1.6; margin: 0; font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, 'Helvetica Neue', Arial, sans-serif;">
      Don't see the funds? <a href="${data.frontendUrl || process.env.FRONTEND_URL || 'http://localhost:3000'}/contact" style="color: #4B5563; text-decoration: underline;">Contact support</a>
    </p>
  `;

  return baseEmailTemplate(content, {
    preheader: `${sym}${fmt(data.amount)} payout for ${data.storeName} has been processed.`,
